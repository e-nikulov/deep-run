...
{
// Public Project 949A displacement figures are not perfectly uniform. Keep the commonly cited
// Apalkov/Deepstorm-style 14,700 t surfaced / 19,400 t submerged pair as source-reference data only.
// The normal-gameplay hydrostatic model is calibrated independently below against the production hull
// waterline: at maximum buoyancy and zero trim the level boat must leave the bow planes 0.50 m clear of
// flat water while both propellers remain submerged. The calibrated full-submerged mass is GAME POLICY,
// not a claim about a classified loading condition.
...
inline constexpr float AnteyGameplayFullSubmergedMassKg = 20'622'264.0F;
inline constexpr float AnteyMainBallastWaterCapacityKg =
    AnteyGameplayFullSubmergedMassKg - AnteyPublicSurfaceDisplacementMassKg;
...
// Compatibility name retained for existing acoustic/runtime code: this is the calibrated normal-gameplay
// fully submerged neutral mass, distinct from the 19,400 t public-reference figure above.
...
static_assert(AnteyCanonicalFullSubmergedMassKg == 20'622'264.0F);
static_assert(AnteyMainBallastWaterCapacityKg == 5'922'264.0F);
