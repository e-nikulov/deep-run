...
constexpr std::array<Marine::ControlSurfaceComponent, 2> M2ControlSurfaces{{
    {.bodyLocalPositionMeters = {32.0F, 0.0F, 0.0F}, .maxEffectiveLiftAreaSquareMeters = 40.0F},
    {.bodyLocalPositionMeters = {-32.0F, 0.0F, 0.0F}, .maxEffectiveLiftAreaSquareMeters = 40.0F}}};
    ...
    const Physics::PhysicsVector3& samplePosition)
{
    ...
    const double lateralSquared = (std::max)(0.0, magnitude * magnitude - alongOutwardNormal * alongOutwardNormal);
...
{
    return std::abs(value[0] - 1.0F) <= 1.0e-5F && std::abs(value[1]) <= 1.0e-5F &&
           std::abs(value[2]) <= 1.0e-5F && std::abs(value[3]) <= 1.0e-5F;
}
    ...
    const bool verifyDistinctUploads)
{
    ...
    {
        return std::unexpected("physical playground production Antey definition load failed: " + productionDefinition.error());
    }
    ...
    if (productionLengthMeters < IG1BProductionLengthMinimumMeters || productionLengthMeters > IG1BProductionLengthMaximumMeters ||
        (*model)->nodes.empty() || (*model)->primitives.empty() || (*model)->materials.size() != 2U)
    {
    ...
    if (collisionProxy.shape != Submarine::ProductionProxyShape::Box || buoyancyProxy.shape != Submarine::ProductionProxyShape::Box ||
        collisionProxy.semanticId != "collision.primary" || buoyancyProxy.semanticId != "buoyancy.primary")
    {
    ...
    const Physics::PhysicsQuaternion collisionProxyOrientation = ToPhysicsQuaternionWxyz(collisionProxy.orientationQuaternionWxyz);
    const Physics::PhysicsQuaternion buoyancyProxyOrientation = ToPhysicsQuaternionWxyz(buoyancyProxy.orientationQuaternionWxyz);
    ...
    if (!collisionCenter.IsFinite() || !collisionHalfExtents.IsFinite() || collisionHalfExtents.x <= 0.0F ||
        collisionHalfExtents.y <= 0.0F || collisionHalfExtents.z <= 0.0F)
    {
    ...
    if (!floraUpload || !floraDraws || flora->patchCount != M3UnderwaterFloraPatchCount ||
        ...
        flora->renderGeometry.primitives.size() != 1U || flora->renderGeometry.nodes.size() != 1U || floraDraws->size() != 1U ||
        floraVertexCount != 960U || floraIndexCount != 1'440U || floraIndexCount / 3U > M3UnderwaterFloraTriangleBudget ||
        !floraUpload->stats.uploadCompleted || !floraUpload->handle.IsValid() || !renderer.IsGpuModelValid(floraUpload->handle) ||
    ...
    if (!iceUpload || !iceDraws || ice->formations.size() != M3UnderwaterIceFormationCount ||
        ...
        ice->renderGeometry.primitives.size() != 1U || ice->renderGeometry.nodes.size() != 1U || iceDraws->size() != 1U ||
        iceVertexCount != 126U || iceIndexCount != 240U || iceIndexCount / 3U > M3UnderwaterIceTriangleBudget ||
        !iceUpload->stats.uploadCompleted || !iceUpload->handle.IsValid() || !renderer.IsGpuModelValid(iceUpload->handle) ||
    ...
    if (!initialDepthSample || std::abs(initialDepthSample->signedDepthMeters - M2InitialSubmarineDepthMeters) > 1.0e-3F)
    {
    ...
    const Physics::PhysicsVector3 propulsorBodyLocalPosition = ShiftProductionPointToBodyLocal(
        M2PropulsorBodyLocalPositionMeters, collisionCenter);
    ...
    const auto initialBuoyancy = Marine::BuoyancySystem::Calculate(
        *water, buoyancy,
        Marine::BuoyancyPose{.worldPositionMeters = initialState->position, .worldOrientation = initialState->orientation},
        *gravityMagnitude);
    if (!initialBuoyancy)
    {
        (void)physics.DestroyBody(body, &physicsError);
    ...
    PlaygroundLog().Info(Diagnostics::LogCategory::Render,
        "Environment section ready: " + seabedSection_->id.value + ", bounds " + FormatBounds(seabedSection_->bounds) +
            ", terrain vertices " + std::to_string(terrainVertexCount) + ", terrain triangles " +
            std::to_string(terrainIndexCount / 3U) + ", rock vertices " + std::to_string(rockVertexCount) +
            ", rock triangles " + std::to_string(rockIndexCount / 3U) + ", rocks " +
            std::to_string(seabedSection_->rocks.size()) + ", environment draws " + std::to_string(seabedDraws_.size()) +
            ", flora plants " + std::to_string(floraField_->plants.size()) + ", flora triangles " +
            std::to_string(floraIndexCount / 3U) + ", ice formations " + std::to_string(iceField_->formations.size()) +
            ", ice triangles " + std::to_string(iceIndexCount / 3U) + ", GPU upload success");
    return {};
}
    ...
    const HapticEventSink& hapticEventSink)
{
    ...
    const auto buoyancyResult = Marine::BuoyancySystem::Calculate(
        *water_, buoyancy_, {.worldPositionMeters = state->position, .worldOrientation = state->orientation}, *gravityMagnitude);
    if (!buoyancyResult)
    ...
    if (!dragResult)
        return std::unexpected("physical playground hydrodynamic drag calculation failed: " + dragResult.error().message);
    ...
    if (!propulsionResult)
        return std::unexpected("physical playground propulsion calculation failed: " + propulsionResult.error().message);
    ...
    if (!propulsionForceWorld)
        return std::unexpected("physical playground propulsion force transform failed: " + propulsionForceWorld.error());
    ...
    if (!propulsorWorldPosition)
        return std::unexpected("physical playground propulsor position transform failed: " + propulsorWorldPosition.error());
    const auto nextPresentationAngle = AdvancePropellerPresentationAngle(
        propellerPresentationAngleRadians_, propulsionState_.shaftRpm, propulsionResult->nextState.shaftRpm, fixedDeltaSeconds);
    if (!nextPresentationAngle)
        return std::unexpected("physical playground propeller presentation advance failed: " + nextPresentationAngle.error());
    ...
    for (std::size_t index = 0; index < controlResults.size(); ++index)
    {
        ...
        if (!physics_->AddForceAtWorldPosition(
                physicsBody_, controlResults[index].forceNewtons, controlResults[index].worldPositionMeters, &controlForceError))
            return std::unexpected("physical playground " + std::string(M2ControlSurfaceNames[index]) +
        ...
        if (hapticEventSink)
        {
            ...
            catch (const std::exception& exception)
            {
                if (!loggedHapticFailure_)
                {
                    ...
                    PlaygroundLog().Warning(Diagnostics::LogCategory::Input,
    ...
    if (!loggedFirstFixedSample_ || (!loggedLaterFixedSample_ && fixedTickCount_ >= M2LaterDiagnosticFixedTick))
    {
        ...
        PlaygroundLog().Info(Diagnostics::LogCategory::Physics,
            ...
            ", velocity " + FormatVector(state->linearVelocity) + ", pitch Z " + std::to_string(pitchDegrees) +
            " deg, angular velocity " + FormatVector(state->angularVelocity) + ", signed depth " +
            ...
            FormatVector(buoyancyResult->totalForceNewtons) + ", drag force " + FormatVector(dragResult->forceNewtons) +
            ", drag torque " + FormatVector(dragResult->torqueNewtonMeters) + ", requested drive " +
            std::to_string(command.throttleFraction) + ", depth command " + std::to_string(command.depthCommandFraction) +
            ", available power " + std::to_string(M2AvailablePropulsionPowerFraction) + ", shaft RPM " +
            std::to_string(propulsionState_.shaftRpm) + ", target RPM " + std::to_string(propulsionResult->targetRpm) +
            ", engine haptic intensity " + std::to_string(engineVibrationIntensity) + ", thrust " +
            std::to_string(propulsionResult->thrustNewtons) + " N, bow deflection " +
            std::to_string(controlDeflections[M2BowPlaneIndex]) + ", bow force " +
            FormatVector(controlResults[M2BowPlaneIndex].forceNewtons) + ", bow point " +
            FormatVector(controlResults[M2BowPlaneIndex].worldPositionMeters) + ", stern deflection " +
            std::to_string(controlDeflections[M2SternPlaneIndex]) + ", stern force " +
            FormatVector(controlResults[M2SternPlaneIndex].forceNewtons) + ", stern point " +
            FormatVector(controlResults[M2SternPlaneIndex].worldPositionMeters) + ", gravity magnitude " +
            std::to_string(*gravityMagnitude) + " m/s^2, weight " + std::to_string(weightMagnitude) +
            " N, point fraction range [" + std::to_string(minimumFraction) + ", " + std::to_string(maximumFraction) + ']');
        const float floatPitchDegrees = 2.0F * std::atan2(surfaceFloatState->orientation.z, surfaceFloatState->orientation.w) *
                                        (180.0F / 3.14159265358979323846F);
        PlaygroundLog().Info(Diagnostics::LogCategory::Physics,
            "Physical playground M3-F float fixed sample: beginning SimulationTime " +
            std::to_string(simulationTimeSeconds) + ", position " + FormatVector(surfaceFloatState->position) +
            ", pitch Z " + std::to_string(floatPitchDegrees) + " deg, wave buoyancy force " +
    ...
    const double presentationTimeSeconds) const
{
    if (!modelAsset_.IsValid() || !renderer.IsGpuModelValid(submarineModel_) || !seabedSection_.has_value() ||
        ...
        iceBodies_.size() != M3UnderwaterIceCollisionCount || !renderer.IsGpuModelValid(iceModel_) || iceDraws_.size() != 1U ||
        !faunaField_.has_value() || faunaField_->fish.size() != M3UnderwaterFishCount ||
        faunaField_->renderGeometry.primitives.size() != 1U || !renderer.IsGpuModelValid(faunaModel_) ||
        faunaBaseDraw_.primitiveIndex != 0U || physics_ == nullptr || !physicsBody_.IsValid() ||
        !surfaceFloatModel_.has_value() || surfaceFloatModel_->primitives.size() != 1U ||
    ...
    if (!surfaceFloatModelToWorld)
        return std::unexpected("physical playground M3-F float transform failed: " + surfaceFloatModelToWorld.error());
    ...
    if (!faunaModelToWorld)
        return std::unexpected("physical playground underwater fauna presentation transform failed: " + faunaModelToWorld.error());
    ...
    if (!surfaceFloatWorldBounds)
        return std::unexpected("physical playground M3-F float bounds transform failed: " + surfaceFloatWorldBounds.error());
    const auto faunaWorldBounds = TransformBounds(faunaField_->renderGeometry.bounds, *faunaModelToWorld);
    if (!faunaWorldBounds)
        return std::unexpected("physical playground underwater fauna bounds transform failed: " + faunaWorldBounds.error());
    ...
    const Assets::ModelBounds cameraDepthBounds = CombineBounds(
        CombineBounds(CombineBounds(CombineBounds(CombineBounds(*worldBounds, seabedSection_->renderGeometry.bounds),
            floraField_->renderGeometry.bounds), iceField_->renderGeometry.bounds), *faunaWorldBounds), *surfaceFloatWorldBounds);
    ...
    // The historical M2/M3 benchmark still proves that its untouched centered 600 m camera contains every
    // accepted playground bound. M5-H.2 free navigation intentionally permits cropping for close inspection
    // and panning, so those old composition-fit assertions are not a runtime restriction on the player's view.
    ...
    const auto faunaStats = renderer.DrawModel(faunaModel_, std::span<const Render::ModelDrawInstance>(&faunaDraw, 1U), *camera);
    ...
    if (!loggedRenderPresentation_)
    {
        ...
        if (waterline)
        {
            ...
            PlaygroundLog().Info(Diagnostics::LogCategory::Render,
                ...
                std::to_string(water_->Config().surfaceLevelY) + ", camera target Y " + std::to_string(camera->target.y) +
                ", horizontal span " + std::to_string(camera->width) + ", vertical span " +
                std::to_string(camera->height) + ", normalized waterline from top " + std::to_string(*waterline));
        }
    }
    return Render::ModelDrawStats{
        ...
        .submittedPrimitives = seabedStats->submittedPrimitives + floraStats->submittedPrimitives + iceStats->submittedPrimitives +
                              faunaStats->submittedPrimitives + submarineStats->submittedPrimitives + surfaceFloatStats->submittedPrimitives,
