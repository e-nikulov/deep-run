    ...
    const float initialSubmarineDepthMeters)
{
    ...
    if (!upload->stats.uploadCompleted || !upload->handle.IsValid() || !renderer.IsGpuModelValid(upload->handle))
    {
    ...
    const Render::GerstnerSurfacePresentationParameters gerstnerSurface = BuildGerstnerSurfacePresentation(*water);
    ...
    const std::size_t floraVertexCount = flora->renderGeometry.primitives.empty() ? 0U : flora->renderGeometry.primitives.front().vertices.size();
    const std::size_t floraIndexCount = flora->renderGeometry.primitives.empty() ? 0U : flora->renderGeometry.primitives.front().indices.size();
    if (!floraUpload || !floraDraws || flora->patchCount != M3UnderwaterFloraPatchCount ||
        ...
        floraUpload->stats.indexCount != floraIndexCount || floraDraws->front().modelToWorld.values != Assets::ModelTransform{}.values)
    {
        return std::unexpected("physical playground underwater flora presentation initialization validation failed" +
                               (!floraUpload ? ": " + floraUpload.error() : !floraDraws ? ": " + floraDraws.error() : std::string{}));
    }
    ...
    const std::size_t iceVertexCount = ice->renderGeometry.primitives.empty() ? 0U : ice->renderGeometry.primitives.front().vertices.size();
    const std::size_t iceIndexCount = ice->renderGeometry.primitives.empty() ? 0U : ice->renderGeometry.primitives.front().indices.size();
    if (!iceUpload || !iceDraws || ice->formations.size() != M3UnderwaterIceFormationCount ||
        ...
        iceUpload->stats.indexCount != iceIndexCount || iceDraws->front().modelToWorld.values != Assets::ModelTransform{}.values)
    {
        return std::unexpected("physical playground underwater ice presentation initialization validation failed" +
                               (!iceUpload ? ": " + iceUpload.error() : !iceDraws ? ": " + iceDraws.error() : std::string{}));
    }
    ...
    const std::size_t faunaVertexCount = fauna->renderGeometry.primitives.empty() ? 0U : fauna->renderGeometry.primitives.front().vertices.size();
    const std::size_t faunaIndexCount = fauna->renderGeometry.primitives.empty() ? 0U : fauna->renderGeometry.primitives.front().indices.size();
    if (!faunaUpload || !faunaDraws || fauna->fish.size() != M3UnderwaterFishCount ||
        ...
        faunaUpload->stats.indexCount != faunaIndexCount || faunaDraws->front().modelToWorld.values != Assets::ModelTransform{}.values)
    {
        return std::unexpected("physical playground underwater fauna presentation initialization validation failed" +
                               (!faunaUpload ? ": " + faunaUpload.error() : !faunaDraws ? ": " + faunaDraws.error() : std::string{}));
    }
    ...
    if (!seabedUpload->stats.uploadCompleted || !seabedUpload->handle.IsValid() || !renderer.IsGpuModelValid(seabedUpload->handle) ||
        !seabedDraws || seabedDraws->size() != 2U || seabedUpload->stats.primitiveCount != seabed->renderGeometry.primitives.size() ||
        seabedUpload->stats.vertexCount != environmentVertexCount || seabedUpload->stats.indexCount != environmentIndexCount ||
        seabedDraws->at(0).modelToWorld.values != Assets::ModelTransform{}.values || seabedDraws->at(1).modelToWorld.values != Assets::ModelTransform{}.values)
    {
    ...
    if (!strategicSeabedUpload || !strategicSeabedDraws || strategicSeabed->materials.size() != 1U || strategicSeabed->primitives.size() != 1U ||
        strategicSeabed->nodes.size() != 1U || strategicSeabedDraws->size() != 1U || !strategicSeabedUpload->stats.uploadCompleted ||
        !strategicSeabedUpload->handle.IsValid() || !renderer.IsGpuModelValid(strategicSeabedUpload->handle) || strategicSeabedUpload->stats.primitiveCount != 1U ||
        strategicSeabedUpload->stats.indexCount != strategicSeabed->primitives.front().indices.size() || strategicSeabedDraws->front().modelToWorld.values != Assets::ModelTransform{}.values)
    {
        return std::unexpected("physical playground strategic seabed presentation initialization validation failed" +
                               (!strategicSeabedUpload ? ": " + strategicSeabedUpload.error() : !strategicSeabedDraws ? ": " + strategicSeabedDraws.error() : std::string{}));
    }
    ...
    if (!initialDepthSample || std::abs(initialDepthSample->signedDepthMeters - initialSubmarineDepthMeters) > 1.0e-3F)
    {
        return std::unexpected("physical playground initial placement does not match the authoritative water depth");
    }
    ...
    {
        control.bodyLocalPositionMeters = ShiftProductionPointToBodyLocal(control.bodyLocalPositionMeters, collisionCenter);
    }
    const Physics::PhysicsVector3 propulsorBodyLocalPosition = ShiftProductionPointToBodyLocal(M2PropulsorBodyLocalPositionMeters, collisionCenter);
    ...
    bodyInfo.mass = M2GameAnteyMassTuningKg;
    bodyInfo.position = initialBodyWorldCenter;
    bodyInfo.orientation = {};
    ...
    if (!body.IsValid())
        return std::unexpected("physical playground rigid body creation failed: " + physicsError.message);
    ...
    const double expectedVolume = static_cast<double>(M2GameAnteyMassTuningKg) / static_cast<double>(water->Config().densityKgPerCubicMeter);
    ...
    {
        if (!point.bodyLocalPositionMeters.IsFinite() || !std::isfinite(point.displacedVolumeCubicMeters) || point.displacedVolumeCubicMeters <= 0.0F)
        {
            (void)physics.DestroyBody(body, &physicsError);
            return std::unexpected("physical playground production buoyancy point is invalid");
        }
        const Physics::PhysicsVector3 productionPoint{point.bodyLocalPositionMeters.x + collisionCenter.x, point.bodyLocalPositionMeters.y + collisionCenter.y, point.bodyLocalPositionMeters.z + collisionCenter.z};
    ...
    const auto initialBuoyancy = Marine::BuoyancySystem::Calculate(*water, buoyancy,
        Marine::BuoyancyPose{.worldPositionMeters = initialState->position, .worldOrientation = initialState->orientation}, *gravityMagnitude);
    if (!initialBuoyancy)
    {
        (void)physics.DestroyBody(body, &physicsError);
        return std::unexpected("physical playground initial buoyancy calculation failed: " + initialBuoyancy.error().message);
    }
    ...
    if (initialBuoyancy->points.size() != M2BuoyancyLongitudinalFractions.size() || !allFullySubmerged ||
        !NearlyEqualRelative(initialBuoyancy->totalSubmergedVolumeCubicMeters, expectedVolume, M2InitialBalanceRelativeTolerance) ||
    ...
    {
        (void)physics.DestroyBody(body, &physicsError);
        return std::unexpected("physical playground initial transform cannot be built: " + initialBodyToWorld.error());
    }
    ...
    {
        const auto control = Marine::ControlSurfaceSystem::Calculate(*water, controlSurfaces[index], initialControlKinematics, 0.0F);
        if (!control || !control->worldPositionMeters.IsFinite() || control->forceNewtons != Physics::PhysicsVector3{})
        {
            (void)physics.DestroyBody(body, &physicsError);
            return std::unexpected("physical playground " + std::string(M2ControlSurfaceNames[index]) +
                " control-surface initialization validation failed" + (control ? std::string{} : ": " + control.error().message));
        }
    }
    ...
    PlaygroundLog().Info(Diagnostics::LogCategory::Physics,
        "Environment collision ready: " + seabed->id.value + ", seabed static bodies " + std::to_string(seabedBodies_.size()) +
        ", ice static bodies " + std::to_string(iceBodies_.size()));
    ...
    if (!surfaceFloatUpload || !surfaceFloatDraws || surfaceFloatDraws->size() != 1U || surfaceFloatModel.primitives.size() != 1U ||
        surfaceFloatModel.primitives.front().vertices.size() != 24U || surfaceFloatModel.primitives.front().indices.size() != 36U ||
        !surfaceFloatUpload->stats.uploadCompleted || surfaceFloatUpload->stats.primitiveCount != 1U || surfaceFloatUpload->stats.vertexCount != 24U ||
        surfaceFloatUpload->stats.indexCount != 36U || !surfaceFloatUpload->handle.IsValid() || !renderer.IsGpuModelValid(surfaceFloatUpload->handle))
    {
        return std::unexpected("physical playground M3-F surface-float model initialization failed" +
            (!surfaceFloatUpload ? ": " + surfaceFloatUpload.error() : !surfaceFloatDraws ? ": " + surfaceFloatDraws.error() : std::string{}));
    }

    const auto initialFloatSurface = water->SampleWaveSurface(M3SurfaceFloatInitialReferencePosition, 0.0);
    if (!initialFloatSurface)
        return std::unexpected("physical playground M3-F initial free-surface sample failed: " + initialFloatSurface.error().message);
    Physics::PhysicsDegreesOfFreedom m3FloatDof;
    m3FloatDof.translationX = true; m3FloatDof.translationY = true; m3FloatDof.translationZ = false;
    m3FloatDof.rotationX = false; m3FloatDof.rotationY = false; m3FloatDof.rotationZ = true;
    Physics::DynamicBoxBodyCreateInfo surfaceFloatInfo{
        .halfExtents = M3SurfaceFloatHalfExtents,
        .mass = M3SurfaceFloatMassKg,
        .position = {M3SurfaceFloatInitialReferencePosition.x, initialFloatSurface->surfaceLevelY, M3SurfaceFloatInitialReferencePosition.z},
        .orientation = {}, .gravityEnabled = true, .linearDamping = M3SurfaceFloatLinearDamping,
        .angularDamping = M3SurfaceFloatAngularDamping, .initialLinearVelocity = {}, .initialAngularVelocity = {}, .degreesOfFreedom = m3FloatDof};
    for (const auto& box : ice->collisionBoxes)
        if (AxisAlignedBoxesOverlap(surfaceFloatInfo.position, surfaceFloatInfo.halfExtents, box.position, box.halfExtents))
    ...
    if (!surfaceFloatBody.IsValid())
        return std::unexpected("physical playground M3-F surface-float body creation failed: " + physicsError.message);
    ...
    const Marine::BuoyancyPose referenceFloatPose{.worldPositionMeters = M3SurfaceFloatInitialReferencePosition, .worldOrientation = initialFloatState->orientation};
    const auto referenceFloatBuoyancy = Marine::BuoyancySystem::Calculate(*water, surfaceFloatBuoyancy, referenceFloatPose, *gravityMagnitude);
    const double expectedFloatNeutralVolume = static_cast<double>(M3SurfaceFloatMassKg) / static_cast<double>(water->Config().densityKgPerCubicMeter);
    ...
    for (const Marine::BuoyancyPoint& point : surfaceFloatBuoyancy.points) configuredFloatPotentialVolume += point.displacedVolumeCubicMeters;
    if (!referenceFloatBuoyancy ||
        !NearlyEqualRelative(configuredFloatPotentialVolume, expectedFloatPotentialVolume, M3SurfaceFloatBalanceRelativeTolerance) ||
        !NearlyEqualRelative(referenceFloatBuoyancy->totalSubmergedVolumeCubicMeters, expectedFloatNeutralVolume, M3SurfaceFloatBalanceRelativeTolerance) ||
        !NearlyEqualRelative(referenceFloatBuoyancy->totalForceNewtons.y, expectedFloatWeight, M3SurfaceFloatBalanceRelativeTolerance))
    {
        (void)physics.DestroyBody(surfaceFloatBody);
    ...
    if (const auto waveCalculated = Marine::BuoyancySystem::CalculateWaveSurface(*water, surfaceFloatBuoyancy,
        {.worldPositionMeters = initialFloatState->position, .worldOrientation = initialFloatState->orientation},
        *gravityMagnitude, 0.0, initialFloatWaveBuoyancy); !waveCalculated)
    {
        (void)physics.DestroyBody(surfaceFloatBody);
        return std::unexpected("physical playground M3-F initial wave buoyancy calculation failed: " + waveCalculated.error().message);
    }
    ...
    modelToBody_ = TranslationTransform({-collisionCenter.x, -collisionCenter.y, -collisionCenter.z});
    ...
    if (verifyDistinctUploads)
    {
        ...
        if (!duplicateUpload) return std::unexpected(duplicateUpload.error());
        if (!duplicateUpload->handle.IsValid() || duplicateUpload->handle == submarineModel_ ||
            !renderer.IsGpuModelValid(duplicateUpload->handle) || duplicateUpload->stats != upload->stats)
    ...
    PlaygroundLog().Info(Diagnostics::LogCategory::Physics,
        ...
        ", collision half extents " + FormatVector(collisionHalfExtents) + ", buoyancy center " + FormatVector(ToPhysicsVector(buoyancyProxy.localCenter)) +
        ", buoyancy half extents " + FormatVector(ToPhysicsVector(buoyancyProxy.halfExtents)) + ", COB " + FormatVector(ToPhysicsVector(buoyancyProxy.centerOfBuoyancy)) +
        ", effective Game displacement " + std::to_string(expectedVolume) + " m^3, mass " + std::to_string(M2GameAnteyMassTuningKg) +
        " kg, initial buoyancy " + std::to_string(initialForce.y) + " N, weight " + std::to_string(expectedWeight) + " N");
    PlaygroundLog().Info(Diagnostics::LogCategory::Render,
        "IG1-D production Antey visual ready: requested LOD " + std::to_string(static_cast<std::size_t>(productionLodSelection->requested)) +
        ", selected LOD " + std::to_string(static_cast<std::size_t>(productionLodSelection->selected)) + ", fallback " +
        std::string(productionLodSelection->usedFallback ? "yes" : "no") + ", asset " + std::string(productionLodSelection->assetId.Value()) +
        ", bounds " + FormatBounds(visualBounds) + ", length " + std::to_string(productionLengthMeters) + " m, nodes " + std::to_string((*model)->nodes.size()) +
        ", primitives " + std::to_string((*model)->primitives.size()) + ", materials " + std::to_string((*model)->materials.size()) +
        ", vertices " + std::to_string(upload->stats.vertexCount) + ", indices " + std::to_string(upload->stats.indexCount));
    PlaygroundLog().Info(Diagnostics::LogCategory::Render,
        "Environment section ready: " + seabedSection_->id.value + ", bounds " + FormatBounds(seabedSection_->bounds) +
        ", terrain vertices " + std::to_string(terrainVertexCount) + ", terrain triangles " + std::to_string(terrainIndexCount / 3U) +
        ", rock vertices " + std::to_string(rockVertexCount) + ", rock triangles " + std::to_string(rockIndexCount / 3U) +
        ", rocks " + std::to_string(seabedSection_->rocks.size()) + ", environment draws " + std::to_string(seabedDraws_.size()) +
        ", flora plants " + std::to_string(floraField_->plants.size()) + ", flora triangles " + std::to_string(floraIndexCount / 3U) +
        ", ice formations " + std::to_string(iceField_->formations.size()) + ", ice triangles " + std::to_string(iceIndexCount / 3U) + ", GPU upload success");
    return {};
}
    ...
    const HapticEventSink& hapticEventSink)
{
    ...
    if (physics_ == nullptr) return std::unexpected("physical playground physics world is unavailable");
    if (!physicsBody_.IsValid() || !surfaceFloatBody_.IsValid()) return std::unexpected("physical playground physics body handle is invalid");
    if (!water_.has_value()) return std::unexpected("physical playground water body is unavailable");
    const auto validatedCommand = ValidateVesselCommandState(command);
    if (!validatedCommand) return std::unexpected("physical playground command validation failed: " + validatedCommand.error());
    ...
    const auto facingAdvance = Submarine::AdvanceAnteyFacing(facingState_, turnAroundRequested, fixedDeltaSeconds);
    if (!facingAdvance) return std::unexpected("physical playground 2.5D facing advance failed: " + facingAdvance.error());
    const float facingProjection = Submarine::AnteyLongitudinalForwardProjection(facingAdvance->nextState);

    const auto state = physics_->GetBodyState(physicsBody_);
    if (!state) return std::unexpected("physical playground body state is unavailable during fixed update");
    const auto surfaceFloatState = physics_->GetBodyState(surfaceFloatBody_);
    if (!surfaceFloatState) return std::unexpected("physical playground M3-F float state is unavailable during fixed update");
    const auto gravityMagnitude = GravityMagnitudeForWater(*physics_, *water_, state->position);
    if (!gravityMagnitude) return std::unexpected("physical playground fixed gravity validation failed: " + gravityMagnitude.error());

    const auto buoyancyResult = Marine::BuoyancySystem::Calculate(*water_, buoyancy_,
        Marine::BuoyancyPose{.worldPositionMeters = state->position, .worldOrientation = state->orientation}, *gravityMagnitude);
    if (!buoyancyResult) return std::unexpected("physical playground buoyancy calculation failed: " + buoyancyResult.error().message);

    const auto surfaceFloatCalculated = Marine::BuoyancySystem::CalculateWaveSurface(*water_, surfaceFloatBuoyancy_,
        {.worldPositionMeters = surfaceFloatState->position, .worldOrientation = surfaceFloatState->orientation},
        *gravityMagnitude, simulationTimeSeconds, surfaceFloatBuoyancyResult_);
    if (!surfaceFloatCalculated)
        return std::unexpected("physical playground M3-F wave buoyancy calculation failed: " + surfaceFloatCalculated.error().message);

    const auto dragResult = Marine::HydroDragSystem::Calculate(*water_, hydroDrag_,
        Marine::HydroDragState{.worldOrientation = state->orientation, .worldLinearVelocityMetersPerSecond = state->linearVelocity,
            .worldAngularVelocityRadiansPerSecond = state->angularVelocity});
    if (!dragResult) return std::unexpected("physical playground hydrodynamic drag calculation failed: " + dragResult.error().message);

    const Marine::PropulsionCommand propulsionCommand{.requestedDriveFraction = command.throttleFraction, .availablePowerFraction = M2AvailablePropulsionPowerFraction};
    const auto propulsionResult = Marine::PropulsionSystem::Advance(propulsion_, propulsionState_, propulsionCommand, fixedDeltaSeconds);
    if (!propulsionResult) return std::unexpected("physical playground propulsion calculation failed: " + propulsionResult.error().message);
    ...
    // Direct semantic Depth still drives the real hydrodynamic bow/stern plane deflections. Variable ballast is
    // evaluated separately below and therefore does not make these surfaces produce fictitious zero-flow lift.
    ...
    {
        const auto control = Marine::ControlSurfaceSystem::Calculate(*water_, controlSurfaces_[index], controlKinematics, controlDeflections[index]);
        if (!control)
            return std::unexpected("physical playground " + std::string(M2ControlSurfaceNames[index]) + " control-surface calculation failed: " + control.error().message);
    ...
    const auto variableBallast = Submarine::CalculateVariableBallastDepthControl(
        ...
        vesselWeightNewtons);
    if (!variableBallast)
        return std::unexpected("physical playground variable-ballast depth control failed: " + variableBallast.error());

    const auto propulsionForceWorld = RotateBodyLocalVectorToWorld(state->orientation,
        {propulsionResult->thrustNewtons * facingProjection, 0.0F, 0.0F});
    if (!propulsionForceWorld) return std::unexpected("physical playground propulsion force transform failed: " + propulsionForceWorld.error());
    ...
    const auto propulsorWorldPosition = TransformBodyLocalPointToWorld(state->position, state->orientation, projectedPropulsorBodyLocalPosition);
    if (!propulsorWorldPosition) return std::unexpected("physical playground propulsor position transform failed: " + propulsorWorldPosition.error());
    const auto nextPresentationAngle = AdvancePropellerPresentationAngle(propellerPresentationAngleRadians_, propulsionState_.shaftRpm,
        propulsionResult->nextState.shaftRpm, fixedDeltaSeconds);
    if (!nextPresentationAngle) return std::unexpected("physical playground propeller presentation advance failed: " + nextPresentationAngle.error());
    ...
    for (std::size_t index = 0; index < buoyancyResult->points.size(); ++index)
    {
        ...
        if (!physics_->AddForceAtWorldPosition(physicsBody_, point.forceNewtons, point.worldPositionMeters, &error))
            return std::unexpected("physical playground buoyancy force application failed at point " + std::to_string(index) + ": " + error.message);
    }
    ...
    if (!physics_->AddForceAtWorldPosition(physicsBody_, variableBallast->forceNewtons, state->position, &ballastForceError))
        return std::unexpected("physical playground variable-ballast force application failed: " + ballastForceError.message);
    ...
    for (std::size_t index = 0; index < surfaceFloatBuoyancyResult_.points.size(); ++index)
    {
        ...
        Physics::PhysicsError error;
        if (!physics_->AddForceAtWorldPosition(surfaceFloatBody_, point.forceNewtons, point.worldPositionMeters, &error))
            return std::unexpected("physical playground M3-F float force application failed at point " + std::to_string(index) + ": " + error.message);
    }

    Physics::PhysicsError dragForceError;
    if (!physics_->AddForceAtWorldPosition(physicsBody_, dragResult->forceNewtons, state->position, &dragForceError))
        return std::unexpected("physical playground hydrodynamic drag force application failed: " + dragForceError.message);
    Physics::PhysicsError dragTorqueError;
    if (!physics_->AddTorque(physicsBody_, dragResult->torqueNewtonMeters, &dragTorqueError))
        return std::unexpected("physical playground hydrodynamic drag torque application failed: " + dragTorqueError.message);
    Physics::PhysicsError propulsionForceError;
    if (!physics_->AddForceAtWorldPosition(physicsBody_, *propulsionForceWorld, *propulsorWorldPosition, &propulsionForceError))
        return std::unexpected("physical playground propulsion force application failed: " + propulsionForceError.message);

    for (std::size_t index = 0; index < controlResults.size(); ++index)
    {
        Physics::PhysicsError controlForceError;
        if (!physics_->AddForceAtWorldPosition(physicsBody_, controlResults[index].forceNewtons, controlResults[index].worldPositionMeters, &controlForceError))
            return std::unexpected("physical playground " + std::string(M2ControlSurfaceNames[index]) + " control-surface force application failed: " + controlForceError.message);
    }
    ...
    if (engineVibration)
    {
        ...
        if (hapticEventSink)
        {
            try { hapticEventSink(*engineVibration); }
            catch (const std::exception& exception)
            {
                if (!loggedHapticFailure_)
                {
                    loggedHapticFailure_ = true;
                    PlaygroundLog().Warning(Diagnostics::LogCategory::Input, "Haptic presentation callback failed and was suppressed: " + std::string(exception.what()));
                }
            }
            catch (...)
            {
                ...
                {
                    loggedHapticFailure_ = true;
                    PlaygroundLog().Warning(Diagnostics::LogCategory::Input, "Haptic presentation callback failed and was suppressed");
                }
            }
        }
    }
    ...
    {
        loggedHapticFailure_ = true;
        PlaygroundLog().Warning(Diagnostics::LogCategory::Input, "Engine-vibration semantic feedback rejected and suppressed: " + engineVibration.error());
    }

    ++fixedTickCount_;
    if (!loggedFirstFixedSample_ || (!loggedLaterFixedSample_ && fixedTickCount_ >= M2LaterDiagnosticFixedTick))
    {
        const auto bodyDepth = water_->Sample(state->position);
        if (!bodyDepth) return std::unexpected("physical playground body-center water sample failed: " + bodyDepth.error().message);
        ...
        const float pitchDegrees = 2.0F * std::atan2(state->orientation.z, state->orientation.w) * (180.0F / 3.14159265358979323846F);
        ...
        PlaygroundLog().Info(Diagnostics::LogCategory::Physics,
            std::string(first ? "Physical playground first M2/I2 fixed sample: " : "Physical playground later M2/I2 fixed sample: ") +
            "tick " + std::to_string(fixedTickCount_) + ", position " + FormatVector(state->position) + ", velocity " + FormatVector(state->linearVelocity) +
            ", pitch Z " + std::to_string(pitchDegrees) + " deg, angular velocity " + FormatVector(state->angularVelocity) + ", signed depth " +
            std::to_string(bodyDepth->signedDepthMeters) + " m, submerged volume " + std::to_string(buoyancyResult->totalSubmergedVolumeCubicMeters) +
            " m^3, buoyancy force " + FormatVector(buoyancyResult->totalForceNewtons) + ", variable ballast authority " +
            std::to_string(variableBallast->lowSpeedAuthorityFraction) + ", ballast target V/S " + std::to_string(variableBallast->targetVerticalSpeedMetersPerSecond) +
            " m/s, ballast force " + FormatVector(variableBallast->forceNewtons) + ", drag force " + FormatVector(dragResult->forceNewtons) +
            ", drag torque " + FormatVector(dragResult->torqueNewtonMeters) + ", requested drive " + std::to_string(command.throttleFraction) +
            ", depth command " + std::to_string(command.depthCommandFraction) + ", available power " + std::to_string(M2AvailablePropulsionPowerFraction) +
            ", shaft RPM " + std::to_string(propulsionState_.shaftRpm) + ", target RPM " + std::to_string(propulsionResult->targetRpm) +
            ", engine haptic intensity " + std::to_string(engineVibrationIntensity) + ", thrust " + std::to_string(propulsionResult->thrustNewtons) +
            " N, bow deflection " + std::to_string(controlDeflections[M2BowPlaneIndex]) + ", bow force " + FormatVector(controlResults[M2BowPlaneIndex].forceNewtons) +
            ", bow point " + FormatVector(controlResults[M2BowPlaneIndex].worldPositionMeters) + ", stern deflection " + std::to_string(controlDeflections[M2SternPlaneIndex]) +
            ", stern force " + FormatVector(controlResults[M2SternPlaneIndex].forceNewtons) + ", stern point " + FormatVector(controlResults[M2SternPlaneIndex].worldPositionMeters) +
            ", gravity magnitude " + std::to_string(*gravityMagnitude) + " m/s^2, weight " + std::to_string(weightMagnitude) +
            " N, point fraction range [" + std::to_string(minimumFraction) + ", " + std::to_string(maximumFraction) + ']');
        const float floatPitchDegrees = 2.0F * std::atan2(surfaceFloatState->orientation.z, surfaceFloatState->orientation.w) * (180.0F / 3.14159265358979323846F);
        PlaygroundLog().Info(Diagnostics::LogCategory::Physics,
            "Physical playground M3-F float fixed sample: beginning SimulationTime " + std::to_string(simulationTimeSeconds) +
            ", position " + FormatVector(surfaceFloatState->position) + ", pitch Z " + std::to_string(floatPitchDegrees) +
            " deg, wave buoyancy force " + FormatVector(surfaceFloatBuoyancyResult_.totalForceNewtons) + " N, submerged volume " +
    ...
    const double presentationTimeSeconds) const
{
    if (!modelAsset_.IsValid() || !renderer.IsGpuModelValid(submarineModel_) || !seabedSection_.has_value() ||
        ...
        faunaBaseDraw_.primitiveIndex != 0U || physics_ == nullptr || !physicsBody_.IsValid() || !surfaceFloatModel_.has_value() ||
        surfaceFloatModel_->primitives.size() != 1U || !renderer.IsGpuModelValid(surfaceFloatModelGpu_) || !surfaceFloatBody_.IsValid() || !water_.has_value())
        return std::unexpected("physical playground model assets are no longer valid");

    const auto state = physics_->GetBodyState(physicsBody_);
    if (!state) return std::unexpected("physical playground body state became unavailable");
    const auto surfaceFloatState = physics_->GetBodyState(surfaceFloatBody_);
    if (!surfaceFloatState) return std::unexpected("physical playground M3-F float state is unavailable during render");
    const auto bodyToWorld = BuildBodyToWorld(*state);
    if (!bodyToWorld) return std::unexpected(bodyToWorld.error());
    const Assets::ModelTransform facingPresentation = Submarine::BuildAnteyFacingPresentationTransform(facingState_);
    const Assets::ModelTransform modelToWorld = Render::Multiply(Render::Multiply(*bodyToWorld, facingPresentation), modelToBody_);
    const auto surfaceFloatModelToWorld = BuildBodyToWorld(*surfaceFloatState);
    if (!surfaceFloatModelToWorld) return std::unexpected("physical playground M3-F float transform failed: " + surfaceFloatModelToWorld.error());
    const auto faunaModelToWorld = EvaluateUnderwaterFishSchoolPresentation(faunaField_->presentation, presentationTimeSeconds);
    if (!faunaModelToWorld) return std::unexpected("physical playground underwater fauna presentation transform failed: " + faunaModelToWorld.error());
    ...
    if (!surfaceFloatNormal) return std::unexpected(surfaceFloatNormal.error());
    ...
    submarineNodeOverrides.reserve(submarineNodeOverrides.size() + depthPlaneMeshNodeIndices_[M2BowPlaneIndex].size() + depthPlaneMeshNodeIndices_[M2SternPlaneIndex].size());
    for (std::size_t group = 0; group < depthPlaneMeshNodeIndices_.size(); ++group)
    {
        const Assets::ModelTransform postTransform = DepthPlanePostTransform(committedControlSurfaceDeflections_[group], group == M2SternPlaneIndex);
    ...
    const Assets::ModelTransform propellerPostTransform = PropellerPostTransform(propellerPresentationAngleRadians_);
    ...
    for (const auto& [bindingIndex, hubPivotModelSpace] : propellerPresentationBindings_)
        submarineBindingOverrides.push_back({.bindingIndex = bindingIndex, .bindingLocalPostTransform = propellerPostTransform, .modelSpacePivot = hubPivotModelSpace});
    if (activeP700HatchGroup_.has_value() && activeP700HatchOpenProgress_ > 0.0F)
    {
        const auto hatch = std::ranges::find_if(p700HatchBindings_, [&](const auto& value) { return value.first == *activeP700HatchGroup_; });
        if (hatch == p700HatchBindings_.end()) return std::unexpected("physical playground active P-700 hatch binding disappeared");
        submarineBindingOverrides.push_back({.bindingIndex = hatch->second, .bindingLocalPostTransform = P700HatchOpenPostTransform(activeP700HatchOpenProgress_)});
    }
    const auto draws = Render::PrepareModelDraws(*modelAsset_, modelToWorld, submarineNodeOverrides, submarineBindingOverrides);
    if (!draws) return std::unexpected(draws.error());

    const auto worldBounds = TransformBounds(modelAsset_->bounds, modelToWorld);
    if (!worldBounds) return std::unexpected(worldBounds.error());
    const auto surfaceFloatWorldBounds = TransformBounds(surfaceFloatModel_->bounds, *surfaceFloatModelToWorld);
    if (!surfaceFloatWorldBounds) return std::unexpected("physical playground M3-F float bounds transform failed: " + surfaceFloatWorldBounds.error());
    const auto faunaWorldBounds = TransformBounds(faunaField_->renderGeometry.bounds, *faunaModelToWorld);
    if (!faunaWorldBounds) return std::unexpected("physical playground underwater fauna bounds transform failed: " + faunaWorldBounds.error());
    const Assets::ModelVector3 target{initialBodyWorldCenter_.x, initialBodyWorldCenter_.y, initialBodyWorldCenter_.z};
    const Assets::ModelBounds cameraDepthBounds = CombineBounds(CombineBounds(CombineBounds(CombineBounds(CombineBounds(*worldBounds,
        seabedSection_->renderGeometry.bounds), floraField_->renderGeometry.bounds), iceField_->renderGeometry.bounds), *faunaWorldBounds), *surfaceFloatWorldBounds);
    const auto camera = Render::BuildFixedWorldSideViewCamera(target, renderer.AspectRatio(), M2GameplayCameraHorizontalSpanMeters, cameraDepthBounds, M5ProductionCameraDepthCantRadians);
    if (!camera) return std::unexpected(camera.error());

    if (!freePresentationCameraFraming_)
    {
        if (!Render::BoundsFitInCamera(*worldBounds, *camera)) return std::unexpected("physical playground production Antey bounds do not fit the fixed camera");
        if (!Render::BoundsFitInCamera(floraField_->renderGeometry.bounds, *camera) || !Render::BoundsFitInCamera(iceField_->renderGeometry.bounds, *camera) ||
            !Render::BoundsFitInCamera(*faunaWorldBounds, *camera) || !Render::BoundsFitInCamera(*surfaceFloatWorldBounds, *camera))
    ...
    const Render::ScenePresentationParameters scenePresentation{
        .depthLighting = {.surfaceLevelYMeters = water_->Config().surfaceLevelY, .attenuationPerMeterRgb = M3DepthAttenuationPerMeterRgb, .deepAmbientRgb = M3DeepAmbientRgb},
        ...
        .fogExtinctionPerMeter = M3FogExtinctionPerMeter, .fogColorRgb = M3FogColorRgb};
    ...
    if (!waterlineViewportY) return std::unexpected("physical playground waterline projection failed: " + waterlineViewportY.error());
    if (freePresentationCameraFraming_)
    {
        const auto skyBands = BuildM5DaySkyPresentationBands(*waterlineViewportY);
        if (!skyBands) return std::unexpected("physical playground day-sky presentation failed: " + skyBands.error());
        for (const auto& band : *skyBands)
        {
            const auto cleared = renderer.ClearViewportRect(band.viewport, band.color);
            if (!cleared) return std::unexpected("physical playground day-sky clear failed: " + cleared.error());
        }
        const auto sunStrips = BuildM5DaySunPresentationStrips(*waterlineViewportY, renderer.AspectRatio());
        if (!sunStrips) return std::unexpected("physical playground day-sun presentation failed: " + sunStrips.error());
        for (const auto& strip : *sunStrips)
        {
            const auto cleared = renderer.ClearViewportRect(strip.viewport, strip.color);
            if (!cleared) return std::unexpected("physical playground day-sun clear failed: " + cleared.error());
        }
    }
    else
    {
        const auto aboveWaterCleared = renderer.ClearViewportRect(Render::ViewportRect{}, M2AboveWaterBackgroundColor);
        if (!aboveWaterCleared) return std::unexpected(aboveWaterCleared.error());
    }

    const auto underwaterRegion = UnderwaterRegionForSurface(*camera, water_->Config().surfaceLevelY);
    if (!underwaterRegion) return std::unexpected("physical playground scalable underwater region failed: " + underwaterRegion.error());
    if (underwaterRegion->has_value())
    {
        ...
        if (!underwaterCleared) return std::unexpected("physical playground scalable underwater clear failed: " + underwaterCleared.error());
    }
    ...
    if (freePresentationCameraFraming_)
    {
        ...
        const bool localSeabedCoversView = UseDetailedEnvironmentPresentation(camera->width) &&
            HorizontalPresentationBoundsCoverView(seabedSection_->renderGeometry.bounds.minimum.x, seabedSection_->renderGeometry.bounds.maximum.x, camera->target.x, camera->width) &&
            seabedSection_->renderGeometry.bounds.minimum.y < cameraBottomWorldYMeters - LocalSeabedBottomSafetyMarginMeters;
        if (!localSeabedCoversView)
        {
            seabedPresentationDraws = {};
            constexpr bool M5CombatRegionWideAreaBathymetryKnown = true;
            if (UseStrategicSeabedPresentation(camera->width, M5CombatRegionWideAreaBathymetryKnown)) strategicSeabedPresentationDraws = strategicSeabedDraws_;
            else floraPresentationDraws = {};
        }
    }

    if (freePresentationCameraFraming_ && seabedPresentationDraws.empty() && strategicSeabedPresentationDraws.empty())
    {
        const auto gameplayBandBottom = ProjectWorldSurfaceToViewportY(*camera, water_->Config().surfaceLevelY - NormalGameplayMaximumVisibleDepthMeters);
        if (!gameplayBandBottom) return std::unexpected("physical playground deep-water boundary projection failed: " + gameplayBandBottom.error());
        const auto abyssBands = BuildDeepWaterAbyssPresentationBands(*gameplayBandBottom);
        if (!abyssBands) return std::unexpected("physical playground deep-water presentation failed: " + abyssBands.error());
        for (const auto& band : *abyssBands)
        {
            const auto cleared = renderer.ClearViewportRect(band.viewport, band.color);
            if (!cleared) return std::unexpected("physical playground deep-water clear failed: " + cleared.error());
        }
    }

    const Render::GerstnerSurfacePresentationParameters gerstnerPresentation = BuildGerstnerSurfacePresentation(*water_);
    const bool gerstnerCoversView = HorizontalPresentationBoundsCoverView(gerstnerPresentation.minimumX, gerstnerPresentation.maximumX, camera->target.x, camera->width);
    std::expected<Render::GerstnerSurfaceDrawStats, std::string> gerstnerStats = Render::GerstnerSurfaceDrawStats{};
    if (gerstnerCoversView) gerstnerStats = renderer.DrawGerstnerSurface(*camera, simulationTimeSeconds);
    if (!gerstnerStats) return std::unexpected("physical playground Gerstner surface draw failed: " + gerstnerStats.error());

    std::expected<Render::ModelDrawStats, std::string> strategicSeabedStats = Render::ModelDrawStats{};
    if (!strategicSeabedPresentationDraws.empty()) strategicSeabedStats = renderer.DrawModel(strategicSeabedModel_, strategicSeabedPresentationDraws, *camera);
    if (!strategicSeabedStats) return strategicSeabedStats;
    std::expected<Render::ModelDrawStats, std::string> seabedStats = Render::ModelDrawStats{};
    if (!seabedPresentationDraws.empty()) seabedStats = renderer.DrawModel(seabedModel_, seabedPresentationDraws, *camera);
    if (!seabedStats) return seabedStats;
    std::expected<Render::ModelDrawStats, std::string> floraStats = Render::ModelDrawStats{};
    if (!floraPresentationDraws.empty()) floraStats = renderer.DrawModel(floraModel_, floraPresentationDraws, *camera);
    if (!floraStats) return floraStats;
    std::expected<Render::ModelDrawStats, std::string> iceStats = Render::ModelDrawStats{};
    if (!icePresentationDraws.empty()) iceStats = renderer.DrawModel(iceModel_, icePresentationDraws, *camera);
    if (!iceStats) return iceStats;
    std::expected<Render::ModelDrawStats, std::string> faunaStats = Render::ModelDrawStats{};
    if (!faunaPresentationDraws.empty()) faunaStats = renderer.DrawModel(faunaModel_, faunaPresentationDraws, *camera);
    if (!faunaStats) return faunaStats;
    const auto submarineStats = renderer.DrawModel(submarineModel_, *draws, *camera);
    if (!submarineStats) return submarineStats;
    const auto surfaceFloatStats = renderer.DrawModel(surfaceFloatModelGpu_, std::span<const Render::ModelDrawInstance>(&surfaceFloatDraw, 1U), *camera);
    if (!surfaceFloatStats) return surfaceFloatStats;

    const bool particleFieldCoversView = HorizontalPresentationBoundsCoverView(M3UnderwaterParticleField.minimumWorldPosition[0],
        M3UnderwaterParticleField.maximumWorldPosition[0], camera->target.x, camera->width);
    std::expected<Render::SuspendedParticleDrawStats, std::string> particleStats = Render::SuspendedParticleDrawStats{};
    if (particleFieldCoversView) particleStats = renderer.DrawSuspendedParticleField(*camera);
    if (!particleStats) return std::unexpected("physical playground particle draw failed: " + particleStats.error());
    if (!loggedRenderPresentation_)
    {
        ...
        if (waterline)
        {
            ...
            PlaygroundLog().Info(Diagnostics::LogCategory::Render,
                "Physical playground M3-E presentation: authoritative surface Y " + std::to_string(water_->Config().surfaceLevelY) +
                ", camera target Y " + std::to_string(camera->target.y) + ", horizontal span " + std::to_string(camera->width) +
                ", vertical span " + std::to_string(camera->height) + ", normalized waterline from top " + std::to_string(*waterline));
        }
    }
    return Render::ModelDrawStats{
        .drawCalls = gerstnerStats->drawCalls + strategicSeabedStats->drawCalls + seabedStats->drawCalls + floraStats->drawCalls + iceStats->drawCalls +
                     faunaStats->drawCalls + submarineStats->drawCalls + surfaceFloatStats->drawCalls + particleStats->drawCalls,
        .submittedPrimitives = strategicSeabedStats->submittedPrimitives + seabedStats->submittedPrimitives + floraStats->submittedPrimitives +
                              iceStats->submittedPrimitives + faunaStats->submittedPrimitives + submarineStats->submittedPrimitives + surfaceFloatStats->submittedPrimitives,
        .submittedIndices = gerstnerStats->indexCount + strategicSeabedStats->submittedIndices + seabedStats->submittedIndices + floraStats->submittedIndices +
                            iceStats->submittedIndices + submarineStats->submittedIndices + surfaceFloatStats->submittedIndices + faunaStats->submittedIndices + particleStats->indexCount};
}
