...
Deep Run uses one Archimedean model across surfaced and submerged operation. The commonly cited 14,700 t surfaced / 19,400 t submerged pair remains recorded as public-source reference data, but the production game's hydrostatic full-submerged state is an explicit calibration: 20,622.264 t, corresponding to 20,119.282 m³ of fixed displaced volume in the canonical 1025 kg/m³ seawater. With the 14,700 t surfaced mass retained, empty main ballast means 5,922.264 t of ballast water has been removed (~40.29% gameplay reserve buoyancy relative to surfaced mass). Jolt rigid-body mass and inertia change with ballast fill. With main ballast empty and zero trim, the level boat settles at ~1.85155 m body-reference depth in flat water: the canonical production bow planes are ~0.50 m above the surface while the highest propeller geometry remains ~0.878 m underwater. With main ballast full, 20,622.264 t is neutrally buoyant when fully immersed. This result comes only from physical mass and fixed displacement volume; there is no surfaced-mode switch, render offset, buoyancy-point offset or reserve-compensation force. In ordinary submerged manoeuvring the main ballast remains flooded: quiet depth changes use bounded trim-water mass at low speed and the stern planes at hydrodynamic speed. Normal main-ballast blowing is armed only in the final near-surface band, while a dive command floods any partly empty main ballast.

Exact Project 949A flood/blow timing is not asserted from public data; the current 40 s full-range transition is explicit GAME POLICY. Small low-speed trim authority is represented as bounded equivalent water mass, not as a direct vertical force. Full-ahead propulsion and immersion-dependent quadratic drag are calibrated to the public 32 kn submerged / 15 kn surfaced canonical without hard velocity clamps. Public sources do not provide a trustworthy Project-949A-specific maximum vertical rate; generic open literature for nuclear submarines quotes roughly 6–9 m/s, so Deep Run does not label that range as an Antey-specific TTX. The current model independently produces ~7.94 m/s as the still-water terminal rise under maximum positive buoyancy and ~6.96 m/s as the 32 kn / 25-degree full-command hydrodynamic trajectory.
...
- with full main ballast the fully immersed 20,622.264 t gameplay state is neutrally buoyant;
- with empty main ballast the 14,700 t state settles naturally at ~1.85155 m body-reference depth in flat water;
- at that level equilibrium the bow planes clear flat water by ~0.50 m and the propellers remain submerged by at least ~0.878 m;
- no render or buoyancy-point offset is permitted to establish the surfaced waterline;
