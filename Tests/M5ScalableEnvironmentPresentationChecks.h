...
[[nodiscard]] inline bool RunM5ScalableEnvironmentPresentationChecks()
{
    ...
    if (NormalGameplaySeaSurfaceReferenceYMeters != 0.0F ||
        ...
        !IsWithinNormalGameplayDepthMeters(0.0F) || !IsWithinNormalGameplayDepthMeters(700.0F) ||
        IsWithinNormalGameplayDepthMeters(700.001F) || !IsWithinNormalGameplayReferenceYMeters(0.0F) ||
        !IsWithinNormalGameplayReferenceYMeters(-700.0F) || IsWithinNormalGameplayReferenceYMeters(-700.001F) ||
        !IsWithinNormalGameplayWaterColumn(-650.0F, 0.0F) || IsWithinNormalGameplayWaterColumn(-701.0F, 0.0F))
        return false;
    ...
    if (std::abs(detailSky - 0.15F) > 0.0001F || std::abs(preThresholdSky - 0.15F) > 0.0001F ||
        std::abs(postThresholdSky - 0.15F) > 0.0001F || std::abs(preThresholdSky - postThresholdSky) > 0.0001F ||
        std::abs(localSky - 0.15F) > 0.0001F || !(transitionSky > localSky && transitionSky < tacticalSky) ||
        ...
        std::abs(AboveWaterFractionForPresentationSpanMeters(2'301.0F) - AboveWaterFractionForPresentationSpanMeters(2'299.0F)) > 0.001F ||
        std::abs(AboveWaterFractionForPresentationSpanMeters(9'001.0F) - AboveWaterFractionForPresentationSpanMeters(8'999.0F)) > 0.001F ||
        std::abs(AboveWaterFractionForPresentationSpanMeters(120'001.0F) - AboveWaterFractionForPresentationSpanMeters(119'999.0F)) > 0.001F)
        return false;
    ...
    // Unknown regions never invent bathymetry. Known combat regions may use the coarse regional profile at
    // any valid span once the caller determines that the bounded detailed section no longer covers the view.
    if (UseStrategicSeabedPresentation(3'600.0F) || !UseStrategicSeabedPresentation(80.0F, true) ||
        !UseStrategicSeabedPresentation(600.0F, true) || !UseStrategicSeabedPresentation(800.0F, true) ||
    ...
    if (!abyssBands || abyssBands->size() != 256U || !noAbyssBands || !noAbyssBands->empty() ||
        ...
        return false;
    for (std::size_t i = 1U; i < abyssBands->size(); ++i)
    {
        const auto& p = (*abyssBands)[i - 1U];
        const auto& c = (*abyssBands)[i];
        if (std::abs(p.viewport.bottom - c.viewport.top) > 0.0001F || c.color.r > p.color.r ||
            c.color.g > p.color.g || c.color.b > p.color.b || std::abs(c.color.b - p.color.b) > 0.01F)
            return false;
    }
    ...
    const auto findPoint = [&tacticalProfile](const float x) -> const StrategicSeabedPresentationPoint*
    {
        for (const auto& point : tacticalProfile)
            if (std::abs(point.xMeters - x) < 0.001F) return &point;
        return nullptr;
    };
    ...
    const std::size_t segments = tacticalProfile.size() - 1U;
    if (!strategic || strategic->primitives.size() != 1U ||
        strategic->primitives[0].vertices.size() != segments * 12U ||
        strategic->primitives[0].indices.size() != segments * 18U ||
    ...
    if (!localTiles || !exactLocalTiles || !pannedTiles || !widerTiles || localTiles->size() != 1U ||
        exactLocalTiles->size() != 1U || !pannedTiles->empty() || !widerTiles->empty() ||
    ...
    const auto localDraws = BuildEnvironmentPresentationDraws(
        std::span<const Render::ModelDrawInstance>(&source, 1U), std::span<const EnvironmentPresentationTile>(*localTiles));
    if (localDraws.size() != 1U || std::abs(localDraws[0].modelToWorld.values[12] - 10.0F) > 0.001F ||
    ...
    const auto scenarioIce = BuildEnvironmentPresentationDraws(
        std::span<const Render::ModelDrawInstance>(&source, 1U), std::span<const EnvironmentPresentationTile>(*localTiles), true);
