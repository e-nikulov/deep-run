...
[[nodiscard]] inline bool RunM5ScalableEnvironmentPresentationChecks()
{
    ...
    if (!HorizontalPresentationBoundsCoverView(-340.0F, 340.0F, 0.0F, 600.0F) ||
        HorizontalPresentationBoundsCoverView(-340.0F, 340.0F, 0.0F, 1'600.0F) ||
        HorizontalPresentationBoundsCoverView(-320.0F, 320.0F, 120.0F, 600.0F))
    {
        return fail("bounded wave/particle detail coverage detection");
    }
    if (!UseDetailedEnvironmentPresentation(600.0F) ||
        !UseDetailedEnvironmentPresentation(9'000.0F) ||
        UseDetailedEnvironmentPresentation(9'001.0F) ||
    ...
    const auto initialTacticalTiles = BuildEnvironmentPresentationTiles(
    ...
    const auto launchOverviewTiles = BuildEnvironmentPresentationTiles(
    ...
    const auto pannedTiles = BuildEnvironmentPresentationTiles(
    ...
    const auto operationalTiles = BuildEnvironmentPresentationTiles(
    ...
    const auto translated = BuildEnvironmentPresentationDraws(
        std::span<const Render::ModelDrawInstance>(&baseDraw, 1U),
        std::span<const EnvironmentPresentationTile>(*initialTacticalTiles));
